...
export const Sidebar: React.FC<SidebarProps> = ({ user, onLogout, isMobileOpen, toggleMobile }) => {
  const location = useLocation();

  const menuItems = [
    { name: 'Início', icon: Home, path: '/dashboard', roles: [UserRole.ADMIN, UserRole.BARBER, UserRole.CLIENT] },
    { name: 'Serviços', icon: LayoutGrid, path: '/dashboard/servicos', roles: [UserRole.CLIENT, UserRole.ADMIN] },
    { name: 'Produtos', icon: ShoppingBag, path: '/dashboard/produtos', roles: [UserRole.CLIENT, UserRole.ADMIN] },
    { name: 'Agendamentos', icon: ClipboardList, path: '/dashboard/agendamentos', roles: [UserRole.CLIENT] },
    { name: 'Meus Dados', icon: UserCheck, path: '/dashboard/dados-pessoais', roles: [UserRole.CLIENT] },
    { name: 'Agenda', icon: Calendar, path: '/dashboard/agenda', roles: [UserRole.ADMIN, UserRole.BARBER] },
    { name: 'Clientes', icon: Users, path: '/dashboard/clientes', roles: [UserRole.ADMIN] },
    { name: 'Equipe', icon: Scissors, path: '/dashboard/staff', roles: [UserRole.ADMIN] },
    { name: 'Financeiro', icon: BarChart3, path: '/dashboard/financeiro', roles: [UserRole.ADMIN] },
    { name: 'Ajustes', icon: Settings, path: '/dashboard/configuracoes', roles: [UserRole.ADMIN, UserRole.BARBER, UserRole.CLIENT] },
    { name: 'Suporte', icon: LifeBuoy, path: '/dashboard/ajuda', roles: [UserRole.ADMIN, UserRole.BARBER, UserRole.CLIENT] },
    ...
    <div className="flex flex-col h-full bg-zinc-950 border-r border-white/5 w-52 shadow-2xl">
      <div className="p-5 pb-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="text-sm font-black tracking-tighter text-white uppercase">{BRAND_NAME}</span>
        </Link>
        <button onClick={toggleMobile} className="lg:hidden text-zinc-600 hover:text-white">
          <X size={16} />
        </button>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon size={14} strokeWidth={isActive ? 2.5 : 2} className={isActive ? 'text-amber-500' : 'text-zinc-600 group-hover:text-zinc-400'} />
              <span className="text-[7.5px] uppercase tracking-[0.15em] font-black">{item.name}</span>
            </Link>
          );
        })}
        ...
        <div className="pt-3 mt-3 border-t border-white/5 mb-4">
           ...
           >
             <LogOut size={14} />
             <span className="text-[7.5px] uppercase tracking-widest font-black">Sair</span>
           </button>
        </div>
      </nav>
      ...
      <div className="p-3">
        <div className="p-2.5 rounded-xl bg-zinc-900/40 border border-white/5 flex items-center gap-2.5">
          ...
          <div className="overflow-hidden">
            <p className="text-[8px] font-black truncate text-white uppercase leading-none">{user.name.split(' ')[0]}</p>
            <p className="text-[6px] text-zinc-600 uppercase font-black tracking-widest mt-1 leading-none">{user.role}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <div className="fixed inset-0 z-[100] lg:hidden">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={toggleMobile} className="absolute inset-0 bg-black/90 backdrop-blur-md" />
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25 }} className="relative h-full w-52">
              <SidebarContent />
            </motion.div>
          </div>
        )}
...
export const Header: React.FC<{ title: string; onToggleSidebar: () => void }> = ({ title, onToggleSidebar }) => (
    ...
    <div className="max-w-[1600px] mx-auto w-full flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="hidden md:flex flex-col border-l border-white/5 pl-4 h-6 justify-center">
             <span className="text-[8px] font-black text-white uppercase tracking-widest leading-none">{title}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2.5 px-4 py-1.5 bg-zinc-900/80 border border-amber-500/20 rounded-full shadow-[0_0_15px_rgba(245,158,11,0.1)]">
         ...
         <span className="text-[9px] font-black text-white uppercase tracking-[0.3em]">{BRAND_NAME}</span>
      </div>
...
export const BottomNavigation: React.FC<{ user: User; onChatClick?: () => void }> = ({ user, onChatClick }) => {
  const location = useLocation();
    ...
    <div className="lg:hidden fixed bottom-6 left-0 right-0 z-[70] px-6 pointer-events-none">
      <div className="max-w-md mx-auto bg-zinc-900/80 backdrop-blur-2xl border border-white/10 rounded-full px-2 py-2 shadow-[0_20px_50px_rgba(0,0,0,0.6)] flex items-center justify-around pointer-events-auto relative overflow-hidden">
        {tabs.map((tab) => {
          ...
          const isActive = !isChat && (location.pathname === tab.path || (tab.path === '/dashboard' && location.pathname === '/dashboard/'));
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center gap-1 transition-all duration-300">
                <div className={`transition-all duration-300 ${
                  isActive ? 'text-amber-500 scale-110' : isChat ? 'text-amber-400' : 'text-zinc-600'
                }`}>
                  {isChat ? (
                    <div className="relative">
                ...
                <span className={`text-[7px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${
                  isActive ? 'text-white' : isChat ? 'text-amber-500/60' : 'text-zinc-600 opacity-60'
                }`}>
                   {tab.name}
                </span>
